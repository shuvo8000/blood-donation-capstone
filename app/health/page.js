...
export const metadata = { title: "Health Check | BloodConnect" };

// Server component: awaits the mock async data source at render time,
// demonstrating that the app can render fetched data end to end.
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-12">
      ...
      <p className="mt-3 max-w-2xl text-sm text-gray-600 sm:text-base">
        ...
        confirms the app can fetch and render data end to end ahead of a
        real backend being connected.
      </p>
      ...
      <div className="mt-8 overflow-x-auto rounded-lg border border-[var(--color-border)]">
        <table className="min-w-full divide-y divide-[var(--color-border)] text-sm">
          ...
          <tbody className="divide-y divide-[var(--color-border)] bg-white">
            {records.map((record) => (
              <tr key={record.id}>
                <td className="px-4 py-3 text-gray-700">{record.label}</td>
